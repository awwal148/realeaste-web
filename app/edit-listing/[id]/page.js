...
import { useAuthStatus } from '@/app/context/useAuthStatus';
...
const Page = () => {
      ...
      const fetchListing = async () => {
        ...
        if (docSnap.exists()) {
          setFormData({
            ...
            images: {}, // Reset the images to allow new uploads
          });
          setLoading(false);
        } else {
  ...
  }, [userCredential, loggedIn, checkingStatus, router, id]);
  ...
  async function onSubmit(e) {
    ...
    if (geolocationEnabled) {
      const response = await fetch(
        `https://maps.googleapis.com/maps/api/geocode/json?address=${address}&key=${process.env.NEXT_PUBLIC_GEO_API_KEY}`
      );
